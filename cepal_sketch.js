...
let iconKiosk, iconRoute, iconSmiling;

function preload() {
    // Carga los archivos SVG
    iconKiosk = loadImage("kiosk.svg");
    iconRoute = loadImage("route.svg");
    iconSmiling = loadImage("smiling.svg");
}
...
function setup() {
    ...
    for (let i = 0; i < cols; i++) {
        for (let j = 0; j < rows; j++) {
            // 20% de chance de dejar un espacio vacío
            ...
            // Se pasa el tamaño original a Tri, y se asigna a targetSize
            triangles.push(new Tri(x, y, size, c, ang));
        }
    }

    // --- ASIGNACIÓN DE LOS NUEVOS ÍCONOS ---
    let availableIcons = [iconKiosk, iconRoute, iconSmiling];
    ...
    if (!moved) {
        ...
        for (let t of triangles) {
            ...
            if (!placed) {
                t.targetX = -200;
                t.targetY = -200;
                t.targetSize = 0; // Para que desaparezca
            }
        }
        ...
        for (let t of triangles) {
            ...
            t.targetSize = t.sizeOriginal; // Usar targetSize
...
function keyTyped() {
    if (key === "s" || key === "S") {
        saveCanvas("grid_triangular_nuevos_iconos", "png");
    }
}
